...
class CnnPolicy(StochasticPolicy):
    def __init__(self, scope, ob_space, ac_space,
        ...
        #self.curiosity_attention(convfeat=convfeat, rep_size=rep_size, enlargement=enlargement)
        #if dynamics_bonus:
        ...
        self.generator = G_conv()
        self.discriminator = D_conv()
        self.cgan_rew(convfeat=convfeat, rep_size=rep_size, enlargement=enlargement)
    ...
    def curiosity_attention(self, convfeat, rep_size, enlargement):
        #convolution network
        for ph_last1 in self.ph_ob_last1.values():
            if len(ph_last1.shape.as_list()) == 5:
                xr = ph_last1[:,1:]
                xr = tf.cast(xr, tf.float32)
                xr = tf.reshape(xr, (-1, *ph_last1.shape.as_list()[-3:]))[:,:,:,-1:]

                xr = tf.nn.leaky_relu(conv(xr, 'c1_att1', nf=convfeat * 1, rf=8, stride=4, init_scale=np.sqrt(2)))
                xr = tf.nn.leaky_relu(conv(xr, 'c1_att2', nf=convfeat * 2, rf=4, stride=2, init_scale=np.sqrt(2)))
                xr = tf.nn.leaky_relu(conv(xr, 'c1_att3', nf=convfeat * 2 * 1, rf=3, stride=1, init_scale=np.sqrt(2)))
                rgbr = [to2d(xr)]
                xr = fc(rgbr[0], 'fc1_att', nh=rep_size, init_scale=np.sqrt(2))

        for ph_last2 in self.ph_ob_last2.values():
            if len(ph_last2.shape.as_list()) == 5:
                xr_last2 = ph_last2[:,1:]
                xr_last2 = tf.cast(xr_last2, tf.float32)
                xr_last2 = tf.reshape(xr_last2, (-1, *ph_last2.shape.as_list()[-3:]))[:,:,:,-1:]

                xr_last2 = tf.nn.leaky_relu(conv(xr_last2, 'c2_att1', nf=convfeat * 1, rf=8, stride=4, init_scale=np.sqrt(2)))
                xr_last2 = tf.nn.leaky_relu(conv(xr_last2, 'c2_att2', nf=convfeat * 2, rf=4, stride=2, init_scale=np.sqrt(2)))
                xr_last2 = tf.nn.leaky_relu(conv(xr_last2, 'c2_att3', nf=convfeat * 2 * 1, rf=3, stride=1, init_scale=np.sqrt(2)))
                rgbr = [to2d(xr_last2)]
                xr_last2 = fc(rgbr[0], 'fc2_att', nh=rep_size, init_scale=np.sqrt(2))

        for ph_last3 in self.ph_ob_last3.values():
            if len(ph_last3.shape.as_list()) == 5:
                xr_last3 = ph_last3[:,1:]
                xr_last3 = tf.cast(xr_last3, tf.float32)
                xr_last3 = tf.reshape(xr_last3, (-1, *ph_last3.shape.as_list()[-3:]))[:,:,:,-1:]

                xr_last3 = tf.nn.leaky_relu(conv(xr_last3, 'c3_att1', nf=convfeat * 1, rf=8, stride=4, init_scale=np.sqrt(2)))
                xr_last3 = tf.nn.leaky_relu(conv(xr_last3, 'c3_att2', nf=convfeat * 2, rf=4, stride=2, init_scale=np.sqrt(2)))
                xr_last3 = tf.nn.leaky_relu(conv(xr_last3, 'c3_att3', nf=convfeat * 2 * 1, rf=3, stride=1, init_scale=np.sqrt(2)))
                rgbr = [to2d(xr_last3)]
                xr_last3 = fc(rgbr[0], 'fc3_att', nh=rep_size, init_scale=np.sqrt(2))

        for ph_last4 in self.ph_ob_last4.values():
            if len(ph_last4.shape.as_list()) == 5:
                xr_last4 = ph_last4[:,1:]
                xr_last4 = tf.cast(xr_last4, tf.float32)
                xr_last4 = tf.reshape(xr_last4, (-1, *ph_last4.shape.as_list()[-3:]))[:,:,:,-1:]

                xr_last4 = tf.nn.leaky_relu(conv(xr_last4, 'c4_att1', nf=convfeat * 1, rf=8, stride=4, init_scale=np.sqrt(2)))
                xr_last4 = tf.nn.leaky_relu(conv(xr_last4, 'c4_att2', nf=convfeat * 2, rf=4, stride=2, init_scale=np.sqrt(2)))
                xr_last4 = tf.nn.leaky_relu(conv(xr_last4, 'c4_att3', nf=convfeat * 2 * 1, rf=3, stride=1, init_scale=np.sqrt(2)))
                rgbr = [to2d(xr_last4)]
                xr_last4 = fc(rgbr[0], 'fc4_att', nh=rep_size, init_scale=np.sqrt(2))
        #输出特征提取后的四个状态
        xr_attention1 = tf.matmul(tf.transpose(xr), xr_last2)
        xr_attention2 = tf.matmul(tf.transpose(xr_last2), xr_last3)
        xr_attention3 = tf.matmul(tf.transpose(xr_last3), xr_last4)
        xr_att1 = tf.nn.softmax(xr_attention1)
        xr_att2 = tf.nn.softmax(xr_attention2)
        xr_att3 = tf.nn.softmax(xr_attention3)
        xr_att = (xr_att1 + xr_att2 + xr_att3)
        #现在的问题：attention要如何训练？可以在policy网络的state输入中引入attention
        self.attention = xr_att

    def cgan_rew(self, convfeat, rep_size, enlargement):
        for ph in self.ph_ob.values():
            if len(ph.shape.as_list()) == 5:  # B,T,H,W,C
                self.batchsize = self.sy_nenvs * (self.sy_nsteps - 1)
        ...
        self.G_sample = self.generator(self.batchsize)      
        self.D_real = self.discriminator(xr)
        ...
        self.forwardloss = self.G_loss
        self.identifyloss = self.D_loss
        self.aux_loss = 100 * (self.identifyloss * (1-0.5) + self.forwardloss * 0.5)
        #这里还有问题，self.int_rew的形式怎样
        self.int_rew = 20 * tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_real, labels=tf.ones_like(self.D_real)),axis=1) + tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_fake, labels=tf.zeros_like(self.D_fake)),axis=1)
    ...
    def define_self_prediction_rew(self, convfeat, rep_size, enlargement):
        ...
        for ph in self.ph_ob.values():
            if len(ph.shape.as_list()) == 5:  # B,T,H,W,C
                ...
                X_r = tf.matmul(X_r, self.attention)
        ...
        for ph in self.ph_ob.values():
            if len(ph.shape.as_list()) == 5:  # B,T,H,W,C
                ...
                X_r_hat = tf.matmul(X_r_hat, self.attention)
...
class G_conv(object):
    def __init__(self):
        self.name = 'G_conv'
        self.size = np.int(7)
        self.channel = 4 # 根据env决定，mario环境为4, atari环境为1
        self.X_dim = 512
        self.hight = 84
        self.width = 84
    
    def __call__(self,batchsize):
        z = tf.random_normal((batchsize, 200), mean=0.0, stddev=2, dtype=tf.float32)
        #z = tf.random_uniform((batchsize, 300), minval=-1.0, maxval=1.0, dtype=tf.float32)

        with tf.variable_scope(self.name) as scope:
            g = tcl.fully_connected(z, np.int(self.size * self.size * 512), activation_fn=tf.nn.relu, normalizer_fn=tcl.batch_norm, weights_initializer=tf.random_normal_initializer(0.02))
            g = tf.reshape(g, (-1, self.size, self.size, 512))
            g = tcl.conv2d_transpose(g, 256, 3, stride=3, activation_fn=tf.nn.relu, normalizer_fn=tcl.batch_norm, padding='SAME', weights_initializer=tf.random_normal_initializer(0.02))
            g = tcl.conv2d_transpose(g, 128, 3, stride=2, activation_fn=tf.nn.relu, normalizer_fn=tcl.batch_norm, padding='SAME', weights_initializer=tf.random_normal_initializer(0.02))
            g = tcl.conv2d_transpose(g, 64, 3, stride=2, activation_fn=tf.nn.relu, normalizer_fn=tcl.batch_norm, padding='SAME', weights_initializer=tf.random_normal_initializer(0.02))
            #g = tcl.conv2d_transpose(g, self.X_dim//16, 3, stride=2, activation_fn=tf.nn.sigmoid, padding='SAME', weights_initializer=tf.random_normal_initializer(0, 0.02))
            g = tf.layers.max_pooling2d(g, 2, strides=2, padding='SAME')
            g = tcl.conv2d_transpose(g, self.channel, 3, stride=2, activation_fn=tf.nn.relu, normalizer_fn=tcl.batch_norm, padding='SAME', weights_initializer=tf.random_normal_initializer(0.02))

#根据生成的图片大小决定
            #g = g = tcl.fully_connected(g, self.hight * self.width * self.channel, activation_fn=tf.nn.relu, normalizer_fn=tcl.batch_norm, weights_initializer=tf.random_normal_initializer(0, 0.02))
            g = tf.reshape(g, (-1, self.hight, self.width, self.channel))
            self.varlist = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.name)
        return g
...
class D_conv(object):
    ...
    def __call__(self, x, reuse=False):
        with tf.variable_scope(self.name) as scope:
            ...
            shared = tcl.conv2d(shared, num_outputs=size * 4, kernel_size=4, stride=2, activation_fn=lrelu, normalizer_fn=tcl.batch_norm)
            shared = tcl.conv2d(shared, num_outputs=size * 8, kernel_size=4, stride=2, activation_fn=lrelu, normalizer_fn=tcl.batch_norm)
            shared = tcl.flatten(shared)
            
            d = tcl.fully_connected(shared, 1, activation_fn=None, weights_initializer=tf.random_normal_initializer(0.02))
            #q = tcl.fully_connected(shared, 128, activation_fn=lrelu, normalizer_fn=tcl.batch_norm)
            #q = tcl.fully_connected(q, 1, activation_fn=None, weights_initializer=tf.random_normal_initializer(0, 0.02))
            self.varlist = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.name)
        return d
