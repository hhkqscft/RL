...
class StochasticPolicy(object):
    def __init__(self, scope, ob_space, ac_space):
        ...
        self.ph_ob_last1 = OrderedDict([(k, tf.placeholder(
                canonical_dtype(self.ph_ob_dtypes[k]),
                (None, None,) + tuple(shapes[k]),
                name=(('obs_last1/%s'%k) if k is not None else 'obs_last1')
            )) for k in self.ph_ob_keys ])
        self.ph_ob_last2 = OrderedDict([(k, tf.placeholder(
                canonical_dtype(self.ph_ob_dtypes[k]),
                (None, None,) + tuple(shapes[k]),
                name=(('obs_last2/%s'%k) if k is not None else 'obs_last2')
            )) for k in self.ph_ob_keys ])
        self.ph_ob_last3 = OrderedDict([(k, tf.placeholder(
                canonical_dtype(self.ph_ob_dtypes[k]),
                (None, None,) + tuple(shapes[k]),
                name=(('obs_last3/%s'%k) if k is not None else 'obs_last3')
            )) for k in self.ph_ob_keys ])
        self.ph_ob_last4 = OrderedDict([(k, tf.placeholder(
                canonical_dtype(self.ph_ob_dtypes[k]),
                (None, None,) + tuple(shapes[k]),
                name=(('obs_last4/%s'%k) if k is not None else 'obs_last4')
            )) for k in self.ph_ob_keys ])
