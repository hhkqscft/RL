...
class InteractionState(object):
    ...
    def __init__(self, ob_space, ac_space, nsteps, gamma, venvs, stochpol, comm):
        ...
        self.buf_ob_last1 = { k: np.zeros(
                            [nenvs, nsteps] + stochpol.ph_ob[k].shape.as_list()[2:],
                            dtype=stochpol.ph_ob_dtypes[k])
                        for k in stochpol.ph_ob_keys }
        self.buf_ob_last2 = { k: np.zeros(
                            [nenvs, nsteps] + stochpol.ph_ob[k].shape.as_list()[2:],
                            dtype=stochpol.ph_ob_dtypes[k])
                        for k in stochpol.ph_ob_keys }
        self.buf_ob_last3 = { k: np.zeros(
                            [nenvs, nsteps] + stochpol.ph_ob[k].shape.as_list()[2:],
                            dtype=stochpol.ph_ob_dtypes[k])
                        for k in stochpol.ph_ob_keys }
        self.buf_ob_last4 = { k: np.zeros(
                            [nenvs, nsteps] + stochpol.ph_ob[k].shape.as_list()[2:],
                            dtype=stochpol.ph_ob_dtypes[k])
                        for k in stochpol.ph_ob_keys }
...
class PpoAgent(object):
    ...
    def __init__(self, *, scope,
                 ob_space, ac_space,
                 stochpol_fn,
                 nsteps=128, nepochs=4, nminibatches=4,
                 gamma=0.99,
                 gamma_ext=0.999,
                 lam=0.95,
                 ent_coef=0.01,
        ...
        with tf.variable_scope(scope):
            ...
            if max_grad_norm:
                _grads, _grad_norm = tf.clip_by_global_norm(grads, max_grad_norm)
            global_grad_norm = tf.global_norm(grads)
            grads_and_vars = list(zip(_grads, vars))
            self._train = trainer.apply_gradients(grads_and_vars)

        #Quantities for reporting.
        self._losses = [loss, pg_loss, vf_loss, entropy, clipfrac, approxkl, maxkl, self.stochpol.aux_loss, self.stochpol.forwardloss, self.stochpol.identifyloss, 
                        global_grad_norm]
        self.loss_names = ['tot', 'pg', 'vf', 'ent', 'clipfrac', 'approxkl', 'maxkl', "auxloss", "G_loss", "D_loss", "gradnorm"]
        self.I = None
    ...
    def update(self):
        ...
        n_rooms = sum(MPI.COMM_WORLD.allgather([len(self.local_rooms)]), [])
            ...
            #logger.info("Rooms visited {self.rooms}")
            #logger.info("Best return {self.best_ret}")
            #logger.info("Best local return {sorted(local_best_rets)}")
            #logger.info("eprews {sorted(eprews)}")
            #logger.info("n_rooms {sorted(n_rooms)}")
            #logger.info("Extrinsic coefficient {self.ext_coeff}")
            #logger.info("Gamma {self.gamma}")
            #logger.info("Gamma ext {self.gamma_ext}")
            #logger.info("All scores {sorted(self.scores)}")
            #logger.logkv("Rooms visited", self.rooms)
            #logger.logkv("Best return", self.best_ret)
            #logger.logkv("Best local return", sorted(local_best_rets))
            #logger.logkv("eprews", sorted(eprews))
            #logger.logkv("n_rooms", sorted(n_rooms))
            #logger.logkv("Extrinsic coefficient", self.ext_coeff)
            #logger.logkv("Gamma", self.gamma)
            #logger.logkv("Gamma ext", self.gamma_ext)
            #logger.logkv("All scores", sorted(self.scores))
        ...
        while epoch < self.nepochs:
            ...
            fd[self.stochpol.ph_ob_last1[None]] = np.squeeze([self.I.buf_ob_last1[None][mbenvinds]])
            fd[self.stochpol.ph_ob_last2[None]] = np.squeeze([self.I.buf_ob_last2[None][mbenvinds]])
            fd[self.stochpol.ph_ob_last3[None]] = np.squeeze([self.I.buf_ob_last3[None][mbenvinds]])
            fd[self.stochpol.ph_ob_last4[None]] = np.squeeze([self.I.buf_ob_last4[None][mbenvinds]])
    ...
    def step(self):
        ...
        for l in range(self.I.nlump):
            ...
            if t == 0:
                for k in self.stochpol.ph_ob_keys:
                    self.I.buf_ob_last1[k][sli, t] = self.I.buf_obs[k][sli, 0]
                    self.I.buf_ob_last2[k][sli, t] = self.I.buf_obs[k][sli, 0]
                    self.I.buf_ob_last3[k][sli, t] = self.I.buf_ob_last2[k][sli, 0]
                    self.I.buf_ob_last4[k][sli, t] = self.I.buf_ob_last3[k][sli, 0]
            else:
                for k in self.stochpol.ph_ob_keys:
                    self.I.buf_ob_last1[k][sli, t] = self.I.buf_obs[k][sli, t]
                    self.I.buf_ob_last2[k][sli, t] = self.I.buf_ob_last1[k][sli, t-1]              
                    self.I.buf_ob_last3[k][sli, t] = self.I.buf_ob_last2[k][sli, t-1]
                    self.I.buf_ob_last4[k][sli, t] = self.I.buf_ob_last3[k][sli, t-1]
        ...
        if t == self.nsteps - 1 and not self.disable_policy_update:
            ...
            fd[self.stochpol.ph_ob_last1[None]] = np.squeeze(self.I.buf_ob_last1[None])
            fd[self.stochpol.ph_ob_last2[None]] = np.squeeze(self.I.buf_ob_last2[None])
            fd[self.stochpol.ph_ob_last3[None]] = np.squeeze(self.I.buf_ob_last3[None])
            fd[self.stochpol.ph_ob_last4[None]] = np.squeeze(self.I.buf_ob_last4[None])
            ...
            self.temp_image = tf.get_default_session().run(self.stochpol.G_sample[0,:,:,:],fd)
            np.save("/home/huhangkai/exp/pic2/temp.npy",self.temp_image)
            np.save("/home/huhangkai/exp/pic2/temp_env.npy",self.I.buf_obs[None][0,0,:,:,:])
            #print("classification result D_fake",self.stochpol.D_fake)
            #print("classification result D_real",self.stochpol.D_real)
