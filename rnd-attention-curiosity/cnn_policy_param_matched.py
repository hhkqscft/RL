    ...
    def cgan_rew(self, convfeat, rep_size, enlargement):
        for ph in self.ph_ob.values():
            if len(ph.shape.as_list()) == 5:  # B,T,H,W,C
                self.batchsize = ph.shape.as_list()[0]
        ...
        print("aaaaaaaaaaaaaaaa",xr,self.G_sample)
        self.D_real, _ = self.discriminator(xr)
        ...
        self.aux_loss = 10 * (self.identifyloss * (1-0.2) + self.forwardloss * 0.2)
        self.int_rew = self.D_loss
...
class G_conv(object):
    def __init__(self):
        self.name = 'G_conv'
        self.size = np.int(7)
        self.channel = 1 # 根据env决定，mario环境为4, atari环境为1
        self.X_dim = 32
        self.hight = 84
        self.width = 84
    
    def __call__(self,batchsize):
        z = np.random.uniform(-1., 1., size=[8,200])
        z = np.float32(z)

        with tf.variable_scope(self.name) as scope:
            g = tcl.fully_connected(z, self.size * self.size * self.X_dim, activation_fn=tf.nn.relu, normalizer_fn=tcl.batch_norm, weights_initializer=tf.random_normal_initializer(0, 0.02))
            g = tf.reshape(g, (-1, self.size, self.size, self.X_dim))
            g = tcl.conv2d_transpose(g, self.X_dim//2, 3, stride=3, activation_fn=tf.nn.relu, normalizer_fn=tcl.batch_norm, padding='SAME', weights_initializer=tf.random_normal_initializer(0, 0.02))
            g = tcl.conv2d_transpose(g, self.X_dim//4, 3, stride=2, activation_fn=tf.nn.relu, normalizer_fn=tcl.batch_norm, padding='SAME', weights_initializer=tf.random_normal_initializer(0, 0.02))
            g = tcl.conv2d_transpose(g, self.X_dim//8, 3, stride=2, activation_fn=tf.nn.relu, normalizer_fn=tcl.batch_norm, padding='SAME', weights_initializer=tf.random_normal_initializer(0, 0.02))
...
class D_conv(object):
    ...
    def __call__(self, x, reuse=False):
        with tf.variable_scope(self.name) as scope:
            ...
            q = tcl.fully_connected(q, 20, activation_fn=None)
            self.varlist = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.name)
        return d, q
